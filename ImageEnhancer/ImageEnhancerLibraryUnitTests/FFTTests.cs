...
namespace ImageEnhancerLibraryUnitTests
{
    ...
    public class FFTTests
    {
        ...
        {
            var tester = new IEImage(new Bitmap(@"D:\GitHub\SE3IA11-Image-Enhancement\ImageEnhancer\TestImage\swanNoise.bmp"));

            var fourierTransformed = Fourier.Transform(tester.GetFullPixelData());

            var fourierOutput = new IEImage(fourierTransformed);

            fourierOutput.GetBitmap().Save(@"D:\GitHub\SE3IA11-Image-Enhancement\ImageEnhancer\TestImage\Output\FFTswanNoise.bmp", ImageFormat.Bmp);


            var inversedPixelData = Fourier.InverseTransform(fourierOutput.GetFullPixelData());
            inversedPixelData.complexPixels = inversedPixelData.complexPixels.Resize(tester.width, tester.height, Complex.Zero);
        ...
        [TestMethod]
        public void RunNonSquareFFT()
        {
            var tester = new IEImage(new Bitmap(@"D:\GitHub\SE3IA11-Image-Enhancement\ImageEnhancer\TestImage\dogOriginal.bmp"));

            var fourierTransformed = Fourier.Transform(tester.GetFullPixelData());

            var output = new IEImage(fourierTransformed).GetBitmap();

            output.Save(@"D:\GitHub\SE3IA11-Image-Enhancement\ImageEnhancer\TestImage\Output\FFTdogOriginal.bmp", ImageFormat.Bmp);
        }

        private IEImage testimage = new IEImage(new Bitmap(@"D:\GitHub\SE3IA11-Image-Enhancement\ImageEnhancer\TestImage\TestImage.bmp"));
        [TestMethod]
        public void RunFFT()
        {
            var tester = new IEImage(new Bitmap(@"D:\GitHub\SE3IA11-Image-Enhancement\ImageEnhancer\TestImage\dogSquare.bmp"));
        ...
        [TestMethod]
        public void CreateFFT()
        {
            var fourierTransformed = Fourier.Transform(testimage.GetFullPixelData());

            var actualResult = fourierTransformed.complexPixels;
